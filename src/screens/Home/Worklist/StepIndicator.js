...
export default function body({ items, handleIndicatorTapped, page }) {
    ...
    const renderIndicator = (item, idx) => {
        ...
        return <TouchableOpacity
            style={[page == idx ? styles.wrapperIndicator : styles.shadowIndicator, Styles.itemCenter, { ...color }]}
            onPress={() => handleIndicatorTapped(item, idx)}
            delayPressIn={0}>
            {page == idx ? <View
                style={[styles.indicator, Styles.itemCenter, { ...color, ...borderColor }]}
            >
                {status === 'pending' ? <SyncIcon /> : status === 'completed' ? <CheckIcon /> : <CheckGreyIcon />}
            </View> : <View />}
            {page != idx ? (status === 'pending') ? <SyncIcon /> : status === 'completed' ? <CheckIcon /> : <CheckGreyIcon /> : <View />}
        </TouchableOpacity>
